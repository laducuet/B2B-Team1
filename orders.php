    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body text-center">
                ...
                <hr>
                <a href="orders.php?itemid=<?= $_GET['itemid'] ?>&feedback=like&buyerUserName=<?= $_GET['buyerUserName'] ?>&buyerId=<?= $_GET['buyerId'] ?>"
                   type="button" class="btn btn-success" data-dismiss="modal">Like <i
                            class="bi bi-hand-thumbs-up-fill"></i></a>
                <a href="orders.php?itemid=<?= $_GET['itemid'] ?>&feedback=dislike&buyerUserName=<?= $_GET['buyerUserName'] ?>&buyerId=<?= $_GET['buyerId'] ?>"
                   type="button" class="btn btn-secondary">Dislike <i class="bi bi-hand-thumbs-down-fill"></i></a>

                <p>Your feedback helps us improve our service.</p>
            </div>
            <div class="modal-footer">
                <a href="orders.php?itemid=<?= $_GET['itemid'] ?>" type="button" class="btn btn-danger"
            ...
            for ($i = 0; $i < $cnt; $i++) {
                ...
                <tr>
                    ...
                    <?php if ($orderDetails[$i]->isShip): ?>
                        <td>
                            <a href="orders.php?itemid=<?= $_GET['itemid'] ?>&acceptOrderId=<?= $orderDetails[$i]->orderId ?>&buyerUserName=<?= $buyer->userName ?>&price=<?= $orderDetails[$i]->orderPrice ?>&quantity=<?= $orderDetails[$i]->quantity ?>&orderDate=<?= $orderDetails[$i]->orderDate ?>&buyerId=<?= $orderDetails[$i]->buyerId ?>&billId=<?= $orderDetails[$i]->billId ?>"
                               class="btn btn-success">Accept</a>
                            <a href="orders.php?itemid=<?= $_GET['itemid'] ?>&deleteOrderId=<?= $orderDetails[$i]->orderId ?>&buyerUserName=<?= $buyer->userName ?>&price=<?= $orderDetails[$i]->orderPrice ?>&quantity=<?= $orderDetails[$i]->quantity ?>&orderDate=<?= $orderDetails[$i]->orderDate ?>&buyerId=<?= $orderDetails[$i]->buyerId ?>&billId=<?= $orderDetails[$i]->billId ?>"
                               class="btn btn-danger">Decline</a>
                            <a href="details.php?itemid=<?= $_GET['itemid'] ?>&buyerId=<?= $orderDetails[$i]->buyerId ?>&billId=<?= $orderDetails[$i]->billId ?>"
